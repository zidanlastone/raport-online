...
export const getScoreDetail = (student, competency) => (dispatch, getState) => {
  axios
    .get(`/api/scores/${student}/${competency}`, tokenConfig(getState))
    .then(res =>
      dispatch({
...
export const addScores = Scores => (dispatch, getState) => {
  axios
    .post("/api/scores", Scores, tokenConfig(getState))
    .then(res =>
      dispatch({
        type: ADD_SCORE,
        payload: res.data
...
export const deleteScore = id => (dispatch, getState) => {
  axios
    .delete(`/api/nilai/${id}`, tokenConfig(getState))
    .then(res =>
      dispatch({
        type: DELETE_SCORE,
        payload: id
